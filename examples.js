/*
  ...
  study these examples closely. nothing to write,  just to study.

  then set examples.iReadTheInstructions to true;
...
examples.forEach((entry, index, arr) => {
  arr[index] = fetching(entry);
})
const liveStudiedExamples = liveStudy(examples, 'Examples');
document.body.appendChild(liveStudiedExamples.container);


// const fetchingExamples = examples.map(fetching);
// fetchingExamples.iReadTheInstructions = examples.iReadTheInstructions;
